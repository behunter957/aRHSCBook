...
public class BookDoublesActivity extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
				...
				.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener() {
					...
					public void onItemSelected(AdapterView<?> parentView,
						...
						Log.i("spinner selected",
								spinnerAdapter.getItem(position).toString());
						targetCourt.setEvent(spinnerAdapter.getItem(position)
								.toString());
					}
		...
		cancelButton.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View v) {
				...
				returnIntent.putExtra("reason", "cancelled by user"); // if http
																		// update
																		// failed
																		// then
																		// return
			...
			public void onCheckedChanged(RadioGroup group, int checkedId) {
				...
				if (((RadioButton) findViewById(checkedId)).isChecked()) {
					...
					case R.id.guest2:
						...
						break;
					case R.id.member2:
						Log.i("BookSinglesActivity",
								"button for member clicked");
	...
	public void onActivityResult(int requestCode, int resultCode, Intent data) {
		...
		if (requestCode == SELECT_PLAYER2) {
			...
			if (resultCode == android.app.Activity.RESULT_OK) {
				Gson gson = new Gson();
				RHSCMember player = gson.fromJson(
						data.getExtras().getString("player"), RHSCMember.class);
				RadioButton playerButton = (RadioButton) findViewById(R.id.member2);
			...
			if (resultCode == android.app.Activity.RESULT_CANCELED) {
				Log.i("return from select player2",
						data.getExtras().getString("reason"));
				((RadioGroup) findViewById(R.id.player2)).check(R.id.tbd2);
			}
		}
		if (requestCode == SELECT_PLAYER3) {
			...
			if (resultCode == android.app.Activity.RESULT_CANCELED) {
				Log.i("return from select player3",
						data.getExtras().getString("reason"));
				((RadioGroup) findViewById(R.id.player3)).check(R.id.tbd3);
			}
		}
		if (requestCode == SELECT_PLAYER4) {
			...
			if (resultCode == android.app.Activity.RESULT_CANCELED) {
				Log.i("return from select player4",
						data.getExtras().getString("reason"));
				((RadioGroup) findViewById(R.id.player4)).check(R.id.tbd4);
			}
		}
	}
	...
	private boolean lockCourt(String booking, String uid) {
		...
		try {
			...
			try {
				...
				Log.e("Getter", "ClientProtocolException on ".concat(targetURI
						.toString()));
				e.printStackTrace();
			} catch (IOException e) {
	...
	private class RHSCUnlockCourtTimeTask extends AsyncTask<String, Void, Void> {
		...
		protected Void doInBackground(String... parms) {
			...
			returnValue = (parms[1].equals("success") ? RESULT_OK
					: RESULT_CANCELED);
			StringBuilder builder = new StringBuilder();
			HttpClient client = new DefaultHttpClient();
			HttpGet httpGet = new HttpGet(targetURI);
	...
	private class RHSCBookCourtTimeTask extends AsyncTask<String, Void, String> {
		...
		public URI getRequestURI(String[] parms) {
			String myURL = String
					.format("http://%s/Reserve20/IOSUpdateBookingJSON.php?b_id=%s&player1=%s&player2=%s&player3=%s&player4=%s&uid=%s&channel=%s&courtEvent=%s",
		...
		protected String doInBackground(String... parms) {
			...
			try {
				...
				if (statusCode == 200) {
					...
					try {
						JSONObject jObj = new JSONObject(builder.toString());
						Log.i("book doubles result",
								jObj.has("result") ? jObj.getString("result")
